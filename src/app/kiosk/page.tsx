...
export default async function KioskPage({
  ...
  if (!session?.user) {
    redirect(`/login?callbackUrl=${encodeURIComponent(`/kiosk?token=${token ?? ""}`)}`);
  }
    ...
    return (
      <div className="flex flex-1 flex-col items-center justify-center py-12">
        <div className="max-w-md rounded-2xl border border-stone-200 bg-white p-6 text-center shadow-sm dark:border-zinc-700 dark:bg-zinc-900/90">
          <p className="font-semibold text-stone-900 dark:text-zinc-50">Employee account required</p>
          <p className="mt-2 text-sm text-stone-600 dark:text-zinc-400">
            Please sign in with an employee account to mark attendance.
          </p>
        </div>
      </div>
    );
